...
一个面向体检报告、食物频率调查、体成分报告、影像报告和通用文档的本地工作台。项目把 PDF 或图片发送到可配置的 OCR 服务，在本机完成结构化解析、人工核对和结果导出；业务数据、原始文件与运行日志均不会提交到代码仓库。
...
- 影像识别：批量识别超声 PDF 中的姓名、性别、年龄、超声所见、超声诊断和右下角检查时间；主版面漏正文时自动补做正文裁剪 OCR，日期不完整时保留原值并提示人工核对，核对后导出 Excel。
- 通用识别：识别不属于现有医疗业务的 PDF 或图片，不做字段与模板限制；可选择返回 Excel、Markdown 或标准 JSON，多文件结果自动打包为 ZIP。
...
本地 FastAPI（8000） ──► 可配置的 OCR API（/parse-file）
        ...
        ├── 食物频率解析
        ├── 体成分基础资料、去脂体重提取与姓名合并
        ├── 超声影像字段提取、人工核对与 Excel 导出
        ├── 通用文档 OCR 与 Excel / Markdown / JSON 导出
        └── 营养 PDF / Excel 导出
```
