...
<template>
  <main class="page-shell">
    ...
    <section class="card service-card">
      <div class="section-title"><div><span class="step">1</span><h2>云端 OCR 服务</h2></div><div class="title-actions"><button class="secondary-button mini-button" type="button" @click="openNormalRangeSettings">正常范围设置</button><button class="text-button" type="button" @click="saveServerUrl">保存地址</button></div></div>
      <label class="field-label" for="server-url">主机地址 / API 地址</label>
      <div class="input-row"><input id="server-url" v-model.trim="serverUrl" class="text-input" placeholder="https://your-host:8443 或完整 /parse-file 地址" /><button type="button" class="secondary-button" :disabled="checkingServer || !serverUrl" @click="testOcr">{{ checkingServer ? '检查中…' : '测试连接' }}</button></div>
      <div class="recognition-options">
        <label class="parse-mode">识别结果解析来源 <select v-model="parseMode" @change="saveParseMode"><option value="markdown">Markdown（推荐：保留表格列）</option><option value="excel">Excel</option></select></label>
      ...
      <p class="mode-help">快速模式保留每页主 OCR，并只在每名受检者的第一页缺姓名时补识别；精确模式会对更多缺姓名页面补识别。</p>
      <p v-if="serverMessage" :class="['server-state', serverOk ? 'ok' : 'error']">{{ serverMessage }}</p>
    </section>

    <div v-if="normalRangeSettingsOpen" class="settings-backdrop" @click.self="closeNormalRangeSettings">
    ...
    <section class="card">
      <div class="section-title"><div><span class="step">2</span><h2>选择批次文件</h2></div></div>
      <div class="upload-actions">
        <label class="drop-zone compact" :class="{ ready: templateFile || lastTemplate.exists }"><input type="file" accept=".xlsx,.xlsm" @change="selectTemplate" /><strong>{{ templateFile ? templateFile.name : (lastTemplate.exists ? `沿用上次模板：${lastTemplate.filename}` : '选择 Excel 汇总模板') }}</strong><span>{{ templateFile ? '本次上传后会替换默认模板' : (lastTemplate.exists ? `已保存于本机：${lastTemplate.path}` : '例如：检验结果_录入表(1).xlsx') }}</span></label>
    ...
    <section v-if="people.length" class="card people-card">
      <div class="section-title"><div><span class="step">3</span><h2>逐人核对与人工修正</h2></div><button class="primary-button" :disabled="savingReview" @click="saveReview">{{ savingReview ? '正在保存…' : '保存人工修改并下载 Excel' }}</button></div>
...
async function testOcr() {
  ...
  try { const body = new FormData(); body.append('ocr_url', serverUrl.value); const response = await fetch('/local-api/test-ocr', { method: 'POST', body }); const data = await response.json(); if (!response.ok) throw new Error(data.detail || '服务不可用'); serverOk.value = true; serverMessage.value = data.message; saveServerUrl() }
