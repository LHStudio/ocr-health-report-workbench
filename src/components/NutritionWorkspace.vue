...
function bodyPairLabel(record, person) { const name = person.general?.姓名 || person.id; const score = matchScore(record, person.id); const used = selectedBodyRecordCount(person.id); return `${name}${score ? `（候选 ${score}%）` : ''}${used ? ` ★已选 ${used}` : ''}` }
...
function loadBodyCompositionMatches(data) { bodyCompositionMatches.value = Array.isArray(data.body_composition_pending_matches) ? data.body_composition_pending_matches.map((record) => ({ ...record, corrected_name: record.corrected_name || '' })) : []; bodyCompositionAssignments.value = Object.fromEntries(bodyCompositionMatches.value.map((record) => [record.id, record.suggested_person_id || ''])) }
