<template>
  <div v-if="people.length" class="nutrition-global-save"><span>{{ saving ? '正在保存当前核对信息…' : '修改后请点击保存，关闭或刷新页面后仍可继续配对。' }}</span><button class="primary-button" type="button" :disabled="saving" @click="saveCurrentReview">{{ saving ? '保存中…' : '保存当前核对信息' }}</button></div>
  ...
  <section v-if="people.length" class="card people-card">
    ...
    <section v-if="bodyCompositionHistory.length" class="body-composition-match-panel">
      <div class="subsection-title"><div><h4>确认体成分姓名配对</h4><p>黄色行表示尚未配对、缺少 OCR 姓名或缺少去脂体重。完全同名才默认预选；相似姓名必须人工确认。</p></div><div class="append-actions"><label>筛选 <select v-model="bodyCompositionMatchFilter"><option value="all">全部</option><option value="pending">仅待配对</option><option value="paired">仅已配对</option><option value="missing-name">仅姓名未识别</option></select></label><button class="primary-button" type="button" :disabled="applyingBodyCompositionMatches" @click="applyBodyCompositionMatches">{{ applyingBodyCompositionMatches ? '正在写入…' : '确认配对并写入' }}</button></div></div>
      <div class="mapping-wrap"><table class="mapping-table nutrition-table body-composition-match-table"><thead><tr><th>体成分文件</th><th>OCR 姓名 / 快速更正</th><th>去脂体重 / 手工补录</th><th>配对人员</th></tr></thead><tbody><tr v-for="record in visibleBodyCompositionMatches" :key="record.id" :class="{ 'needs-review-row': bodyRecordNeedsReview(record), 'body-match-applied': record.match_status === 'applied' }"><td><a v-if="record.file_url" :href="record.file_url" download>{{ record.filename }}</a><span v-else>{{ record.filename }}</span></td><td><strong>{{ record.name || '未识别' }}</strong><div v-if="record.match_status !== 'applied'" class="body-name-correction"><input v-model.trim="record.corrected_name" class="cell-input" placeholder="手工录入或改正姓名" @input="autoPairByManualName(record)" /><button class="secondary-button mini-button" type="button" @click="suggestPersonByRecordName(record)">按姓名匹配</button></div></td><td><strong>{{ record.fat_free_mass ? `OCR：${record.fat_free_mass} kg` : 'OCR 未识别' }}</strong><div v-if="record.match_status !== 'applied'" class="body-name-correction"><input v-model.trim="record.corrected_fat_free_mass" class="cell-input" inputmode="decimal" placeholder="手工补录去脂体重（kg）" /></div></td><td><template v-if="record.match_status === 'applied'"><strong class="applied-match">✓ 已写入：{{ pairedPersonLabel(record) }}</strong></template><template v-else><select v-model="bodyCompositionAssignments[record.id]"><option value="">暂不写入</option><option v-for="person in people" :key="person.id" :value="person.id">{{ bodyPairLabel(record, person) }}</option></select><button class="text-button body-name-copy" type="button" :disabled="!bodyCompositionAssignments[record.id] || !(record.corrected_name || record.name)" @click="overwritePersonName(record)">用体成分姓名修正问卷 OCR 姓名</button></template></td></tr></tbody></table></div>
    </section>
...
const bodyCompositionMatches = ref([]); const bodyCompositionHistory = ref([]); const bodyCompositionAssignments = ref({}); const bodyCompositionMessage = ref(''); const bodyCompositionError = ref(false); const bodyCompositionMatchFilter = ref('all')
...
function bodyRecordNeedsReview(record) { return record.match_status !== 'applied' && (!bodyCompositionAssignments.value[record.id] || !String(record.corrected_name || record.name || '').trim() || !effectiveFatFreeMass(record)) }
