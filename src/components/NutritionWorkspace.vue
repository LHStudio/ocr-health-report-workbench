<template>
  ...
  <section v-if="people.length" class="card people-card">
    ...
    <section v-if="bodyCompositionMatches.length" class="body-composition-match-panel">
      ...
      <div class="mapping-wrap"><table class="mapping-table nutrition-table body-composition-match-table"><thead><tr><th>体成分文件</th><th>OCR 姓名 / 快速更正</th><th>去脂体重 / 手工补录</th><th>配对人员</th></tr></thead><tbody><tr v-for="record in visibleBodyCompositionMatches" :key="record.id" :class="{ 'needs-review-row': bodyRecordNeedsReview(record) }"><td><a v-if="record.file_url" :href="record.file_url" download>{{ record.filename }}</a><span v-else>{{ record.filename }}</span></td><td><strong>{{ record.name || '未识别' }}</strong><div class="body-name-correction"><input v-model.trim="record.corrected_name" class="cell-input" placeholder="手工录入或改正姓名" @input="autoPairByManualName(record)" /><button class="secondary-button mini-button" type="button" @click="suggestPersonByRecordName(record)">按姓名匹配</button></div></td><td><strong>{{ record.fat_free_mass ? `OCR：${record.fat_free_mass} kg` : 'OCR 未识别' }}</strong><div class="body-name-correction"><input v-model.trim="record.corrected_fat_free_mass" class="cell-input" inputmode="decimal" placeholder="手工补录去脂体重（kg）" /></div></td><td><select v-model="bodyCompositionAssignments[record.id]"><option value="">暂不写入</option><option v-for="person in people" :key="person.id" :value="person.id">{{ bodyPairLabel(record, person) }}</option></select><button class="text-button body-name-copy" type="button" :disabled="!bodyCompositionAssignments[record.id] || !(record.corrected_name || record.name)" @click="overwritePersonName(record)">用体成分姓名修正问卷 OCR 姓名</button></td></tr></tbody></table></div>
    </section>
...
const bodyCompositionMatches = ref([]); const bodyCompositionAssignments = ref({}); const bodyCompositionMessage = ref(''); const bodyCompositionError = ref(false); const bodyCompositionMatchFilter = ref('all')
...
const visibleBodyCompositionMatches = computed(() => bodyCompositionMatches.value.filter((record) => {
  ...
  if (bodyCompositionMatchFilter.value === 'paired') return paired
...
function loadBodyCompositionMatches(data) { bodyCompositionMatches.value = Array.isArray(data.body_composition_pending_matches) ? data.body_composition_pending_matches.map((record) => ({ ...record, corrected_name: record.corrected_name || '', corrected_fat_free_mass: record.corrected_fat_free_mass || '' })) : []; const savedAssignments = data.body_composition_assignments || {}; bodyCompositionAssignments.value = Object.fromEntries(bodyCompositionMatches.value.map((record) => [record.id, savedAssignments[record.id] || record.suggested_person_id || ''])) }
...
async function applyBodyCompositionMatches() {
  ...
  try {
    ...
    replacePeople(data.people); loadBodyCompositionMatches({ body_composition_pending_matches: data.pending_matches || [], body_composition_assignments: data.assignments || {} }); appendBodyCompositionFiles.value = []; bodyCompositionMessage.value = data.message || '去脂体重已追加。'
